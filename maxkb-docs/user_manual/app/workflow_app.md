...
- 用户输入添加参数的组件类型支持文本框、密码框、单选框、多选框、选项卡、日期、开关、单行选项卡、多行文本框、单行多选卡、模型、知识库和树形选择器。
- 显隐设置：新增标签页，专门配置联动规则
     显示条件：满足所有 / 任意条件时，当前参数才展示；不满足则直接隐藏
     隐藏条件：满足所有 / 任意条件时，当前参数直接隐藏；不满足则正常展示
- 单条条件固定三段式下拉：
  【变量源】 → 【运算符】 → 【匹配值】
  1、变量源下拉可选范围（严格限定，避免循环引用）：
     排序在本参数之前、已配置完成的其他用户输入参数
  2、运算符全集：
     等于/不等于/包含/不包含/为真/不为真/大于/大于等于/小于/小于等于       
  3、匹配值下拉联动：
...
- 视觉：高级智能体的AI对话节点全面支持多模态输入；用户在对话过程中可以同时传入文本、图片等多种形式的内容，模型将综合多模态信息进行理解与回答。
      
     - 默认状态：功能关闭，节点仅以纯文本大模型逻辑运行，无多模态解析能力；
     - 开启后：支持对话上传图片、视频素材，节点将切换为多模态模型执行逻辑；
     - 注意事项：若开启视觉功能，但所选模型不支持多模态能力，流程执行时会触发报错，请确保选用多模态大模型。
