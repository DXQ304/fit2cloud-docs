...
- 安全漏洞：修复权限校验缺陷，阻断匿名应用会话 Token 越权获取管理员系统 API Key 的高危权限漏洞；
- 知识库：修复上传 PDF 文档时，当文档内容中的标题与正文有相同内容时，段落内容错乱的问题。
:::
...
- 模型：修复豆包的图片生成模型添加报错的问题；
- 应用：修复对话日志数据量较大时，导出对话日志会服务重启的问题；
- 应用：修复对话日志有异常内容时，会导致无法导出对话记录的问题；
- 应用：修复判断器节点无法处理多个条件的问题；
- 登录：修复用户名为中文时无法登录的问题（#4232）；
- 国际化：修复在繁体简体语言下，添加 Xorbits Inference 的视觉模型时描述不正确的问题。
:::
...
- 登录：优化登录系统时针对用户的密码进行了加密。
:::

:::note[问题修复 🌴]

- 问答页面：修复在未完整上传文件后进行提问时，AI 回复报错的问题；
- 应用：修复在对话时偶然出现数据库连接关闭的问题；
- 应用：修复在浮窗对话框中进行提问时，应用的对话日志中未显示 asker 用户的问题；
- 应用：修复思考过程样式显示不正确的问题；
- 应用：修复没有应用权限的用户仍可以通过 API 接口查看应用的对话记录的问题；
- 接口文档：修复若干接口文档显示问题。
:::
...
- 问答页面：用户可直接发送文件或图片进行提问。
:::

:::note[问题修复 🌴]

- 知识库：修复在部分情况下无法访问web知识库文档中的链接；
- 知识库：修复在文档的分段详情中按分段内容搜索，结果不正确的问题；
- 应用：修复高级编排应用的函数节点中参数不回显的问题；
- 应用：修复高级编排应用中节点连接错乱的问题；
- 应用：修复AI回复的内容被包含到MCP的执行结果中的问题；
- 应用：修复表单收集中多次拖拽参数，控制台报错的问题；
- 应用：修复在对话日志中点击开场白设置的快捷问题，会调用应用回答的问题；
- 操作日志：修复在操作日志可以看到用户密码的漏洞；（X-Pack）
- 安装部署：PostgreSQL数据库版本升级到15.14；
- 接口文档：修复若干已知问题。
:::
...
:::note[问题修复 🌴]
 ...
 -  知识库（X-Pack）：修复飞书知识库文档在“设置”功能中修改“命中处理方式”选项时保存报错的问题；
 -  知识库：修复在文档中添加、删除、修改分段操作后，文档列表的更新时间未同步更新的问题；
 -  函数库：修复内置的数据库查询函数序列化为 JSON 字符串时失败的问题；
 -  问答页面：修复通过快捷键复制粘贴图片时，会覆盖此前上传图片的问题；
 -  问答页面：修复 Firefox 浏览器无法通过拖拽方式上传文件的问题；
 -  问答页面：修复上传的文件名称中含有“&nbsp” 字符时，不显示 URL 的问题；
 -  问答页面：修复应用名称过长导致界面显示错位的问题；
 -  问答页面：修复对话记录超过 20 条后无法展示最新提问的问题；
 -  问答页面：修复开场白中单个英文单词被拆分显示到两行的问题；
 -  应用（X-Pack）：修复简易应用未在“显示设置”功能中勾选“显示历史记录”设置时，问答页面显示异常的问题；
 -  应用：修复高级编排应用中，部分情况下丢失思考过程标签的问题；
 -  应用：修复对话用户并发较多时，特殊情况下会出现数据库文件损坏的问题；
 -  应用：修复在对话日志中自定义查询时间后导出报错的问题；
 -  应用：修复通过 API Key 进行非流式对话时未统计 Token 消耗的问题；
 -  应用：修复指定回复节点输出的表单参数中含有特殊字符导致报错的问题；
 -  应用：修复 MCP 节点的配置信息填写错误时提示信息不正确的问题；
 -  应用：修复使用阿里云百炼的 DeepSeek-R1 模型通过 Streamable HTTP MCP 方式调用 MCP 服务时无法返回内容的问题；
 -  安装部署：修复若干已知问题；
 -  API 文档：修复若干已知问题。
:::
