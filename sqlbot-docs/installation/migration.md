...
SQLBot 的迁移工作建议在同版本的情况下进行。如源服务器上的 SQLBot 版本是 v1.4.0，则目标服务器上请先安装好 v1.4.0 版本的 SQLBot。迁移前请先按 [备份还原](./backup) 做好备份。若使用外置数据库，除复制运行目录外，还需单独迁移 PostgreSQL 中的 `sqlbot` 库。
