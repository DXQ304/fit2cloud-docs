...
| **操作**      | 该行的可执行动作       | 支持**启用/禁用开关**（暂停或恢复调用）、**编辑**（改名称）和**删除**（彻底移除）  |
...
1. 在目标 Key 所在行最右侧的「**操作**」列，单击开关将其**关闭**。
2. 禁用后页面顶部提示「API Key 已禁用」，该 Key 的调用会立即被网关拒绝，但 Key 本身保留在列表里。
3. 需要恢复使用时，再次单击同一开关打开即可，无需重新创建。

<img style={{display:"block",margin:"16px auto",maxWidth:"100%"}} src="/img/api-keys/image4-disable-api-key.png" alt="禁用 API Key"/>

<div style={{textAlign:"center",color:"#8a8f99",fontSize:"13px",margin:"6px 0 20px"}}>图 4  关闭开关后 API Key 被禁用</div>
...
- 业务方已切到新 Key
- 如果只是想「暂停」使用，建议改用[禁用](#_34-启用与禁用-api-key)，随时可以重新启用，无需删除
